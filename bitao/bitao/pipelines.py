    ...
    def do_insert(self, cursor, item):
        ...
        if count_data[0]>0:
            ...
            if item["retweeted_status"] is not None and len(item["retweeted_status"]):
                ...
                cursor.execute(
                    " insert reptile_source(content,publisher_nicke_name,publisher_real_name,publisher_avatar,publish_time,"
                    ...
                    {"content": self.filter_emoji(text).encode('utf-8') if text is not None else "",
                ...
                cursor.execute(" select id from reptile_source where content_id='" + id + "' limit 1 ")
                ...
                if etweeted_status.has_key("pics"):
                    ...
                    for etweeted_pic in etweeted_pics :
                        ...
                        cursor.execute(
                            "insert into reptile_source_pics(reptile_source_id,pic_url,big_pic_url,position_sort) VALUES "
                            "(%(reptile_source_id)s,%(pic_url)s,%(big_pic_url)s,%(sort)s)",
                            {"reptile_source_id": etweeted_data_id[0], "pic_url": etweeted_small, "big_pic_url": etweeted_large,
                ...
                if etweeted_status.has_key("page_info"):
                    ...
                    cursor.execute(
                        ...
                        {"reptile_source_id": etweeted_data_id[0], "video_url": etweeted_video_url, "cover_image_url": etweeted_cover_image_url
                         })
            # 保存微博正文
            cursor.execute(
                ...
                ",publisher_desc,source_platform,source_address,content_id,forward_content_id) "
                " VALUES (%(content)s,%(publisher_nicke_name)s,%(publisher_real_name)s "
                ",%(publisher_avatar)s,%(publish_time)s,%(publisher_identifier)s,%(publisher_desc)s  "
                ",%(source_platform)s,%(source_address)s,%(content_id)s ,%(forward_content_id)s )",
                {"content": self.filter_emoji(item["content"]).encode('utf-8') if item.has_key("content") else "",
                 ...
                 "forward_content_id":etweeted_id if  etweeted_id is not None else ""
                 })
